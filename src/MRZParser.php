...
namespace azerioid\mrz;

/**
 *
 Author bbeycanov
 For Laravel Package azerioid
 * */
...
class MRZParser
{
     ...
     * @return array|null
     */
    public static function parse($mrz): ?array
    {
        ...
        $documentCode = (new MRZParser)->stripPadding($mrz[0]);

        switch ($documentCode) {

            case 'P' :
                return (new MRZParser)->parseMRZPassport($mrz);
            case 'A' :
            case 'C' :
            case 'I' :

                if ($len === 90) {
                    return (new MRZParser)->parseMRZID1($mrz);
                }

                if ($len === 72) {
                    if (substr($mrz, 2, 3) === 'FRA') {
                        return (new MRZParser)->parseMRZFrenchID($mrz);
                    }

                    return (new MRZParser)->parseMRZID2($mrz);
                }
                break;

            case 'V' :
                return (new MRZParser)->parseMRZVisa($mrz);
        }
